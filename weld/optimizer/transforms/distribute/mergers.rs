...
pub fn gen_merge_dicts(result_iter: &Iter, result_ty: Type,
                       builder: &Expr) -> WeldResult<Expr> {
    let inner_params = code_util::new_loop_params(&builder.ty, &Vector(Box::new(result_ty.clone())), builder);
    let outer_params = code_util::new_loop_params(&builder.ty, &result_ty, builder);

    // convert dictionary to a vector
    // lookup_expr used to get dictionary from pointer
    let dict_vec = constructors::tovec_expr(constructors::lookup_expr(constructors::ident_from_param(outer_params[2].clone()).unwrap(),
