...
impl SerDeGen for LlvmGenerator {
    unsafe fn gen_serialize(&mut self,
        ...
        if let Serialize(ref child) = statement.kind {
            ...
            let (serialized, _) = self.gen_serialize_helper(ctx.builder,
                                                       zero,
                                                       child,
                                                       child_ty,
                                                       buffer,
    ...
    unsafe fn gen_deserialize(&mut self, ctx: &mut FunctionContext, statement: &Statement) -> WeldResult<()> {
        ...
        if let Deserialize(ref child) = statement.kind {
            ...
            let _ = self.gen_deserialize_helper(ctx.builder,
                                        zero,
                                        output,
                                        output_ty,
                                        buffer,
        ...
        if !self.serialize_fns.contains_key(ty) {
            ...
            let (updated_buffer, updated_position) = match *ty {
                ...
                Vector(ref elem) if !elem.has_pointer() => {
                    ...
                    let (buffer, position) = self.gen_put_value(builder, vec_size, buffer, run, position)?;
                ...
                Vector(ref elem) => {
                    ...
                    // end of ser.end.
                    let start_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("ser.vec.start"));
                    let loop_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("ser.vec.loop"));
                    let end_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("ser.vec.end"));
                    ...
                    let (start_buffer, start_position) = self.gen_put_value(builder, size, buffer, run, position)?;
                    let zero = self.i64(0);
    ...
    unsafe fn gen_deserialize_helper(&mut self,
        ...
        if !self.deserialize_fns.contains_key(ty) {
            ...
            let updated_position = match *ty {
                Scalar(_) => {
                    let (value, position) = self.gen_get_value(builder, LLVMGetElementType(LLVMTypeOf(output)), buffer, position)?;
                ...
                Vector(ref elem) if !elem.has_pointer() => {
                    ...
                    let (size, position) = self.gen_get_value(builder, size_type, buffer, position)?;
                    let vector = self.gen_new(builder, ty, size, run)?;
                    let zero = self.i64(0);
                    let data_pointer = self.gen_at(builder, ty, vector, zero)?;
                    let position = self.gen_get_values(builder, data_pointer, size, buffer, position)?;
                ...
                Struct(ref tys) => {
                    ...
                    for (i, ty) in tys.iter().enumerate() {
                        ...
                        position = self.gen_deserialize_helper(builder, position, value_pointer, ty, buffer, run)?;
                    }
                    position
                }
                Vector(ref elem) => {
                    ...
                    let start_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("de.vec.start"));
                    let loop_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("de.vec.loop"));
                    let end_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("de.vec.end"));
                    ...
                    let updated_position = self.gen_deserialize_helper(builder, phi_position, value_pointer, elem, buffer, run)?;
                ...
                Dict(ref key_ty, ref value_ty) => {
                    ...
                    let start_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("de.dict.start"));
                    let loop_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("de.dict.loop"));
                    let end_block = LLVMAppendBasicBlockInContext(self.context, function, c_str!("de.dict.end"));
        ...
        Ok(LLVMBuildCall(builder, function, args.as_mut_ptr(), args.len() as u32, c_str!("")))
    }
}
