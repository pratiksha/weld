...
fn partition_to_slice(vec_ident: &Expr<Type>,
                      partition_ident: &Expr<Type>) -> WeldResult<Expr<Type>> {
    exprs::slice_expr(vec_ident.clone(), /* slice each input iter. */
                      exprs::getfield_expr(partition_ident.clone(), 0)?, /* slice start index */
                      exprs::getfield_expr(partition_ident.clone(), 1)?  /* slice size */)
}

/// Call a CUDF to split total_len into nworkers_conf shards.
pub fn gen_partition_call(total_len: &Expr<Type>, nworkers_conf: &i32) -> WeldResult<Expr<Type>> {
    ...
    let partition_expr = exprs::cudf_expr(PARTITION_SYM.to_string(),
                                      vec![total_len.clone(),
                                           nworkers,
                                           increment],
...
fn vec_info_to_shard(vi: &vec_info,
                     ...
                     partition_loop_elt: &Expr<Type>) -> WeldResult<Expr<Type>> {
    if vi.is_sharded {
        exprs::lookup_expr(vi.ident.clone(), partition_loop_idx.clone())
    } else {
        partition_to_slice(&vi.ident, &partition_loop_elt)
    }
}
...
pub fn partitions_to_shards(iters: &Vec<vec_info>,
                            partition_loop_idx: &Expr<Type>,
                            partition_loop_elt: &Expr<Type>) -> WeldResult<(Vec<Expr<Type>>,
                                                                            Vec<Parameter<Type>>)> {
    let mut shards = vec![];
    let mut params = vec![];
